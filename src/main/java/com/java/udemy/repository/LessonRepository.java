...
public interface LessonRepository extends CrudRepository<Lesson, Integer> {
    ...
    // @Query(value = "SELECT s.id, s.lesson_name, s.position,
    // TIME_FORMAT(SEC_TO_TIME(s.length_seconds), "
    // +
    // "'%i:%s') AS video_time, EXISTS(SELECT 1 FROM enroll_progress ep WHERE
    // ep.lesson_id = s.id AND ep.enrollment_id = ?1) "
    // +
    // "AS is_watched FROM lessons s WHERE s.course_id = ?2 ORDER BY s.position",
    // nativeQuery = true)
    // List<Map<String, Object>> getWatchStatusListByEnrollment(Long enrollId,
    // Integer courseId);
